...
// use super::modifier_fn::ReadyFuture;
...
impl AddOrigin {
    ...
    pub(crate) fn to_request_fn(
        self,
    ) -> impl FnOnce(RawRequest<Body>) -> Ready<RawRequest<Body>> + Clone {
        move |req| {
            ...
            head.uri = {
                ...
                uri.scheme = Some(self.scheme);
                uri.authority = Some(self.authority);

                http::Uri::from_parts(uri).expect("valid uri")
            };

            std::future::ready(Request::from_parts(head, body))
        }
    }
}
    ...
    pub(crate) fn to_request_fn(
        ...
        move |mut req| {
            ...
            if let Ok(entry) = req.headers_mut().try_entry(USER_AGENT) {
                // This is to avoid anticipative cloning
                match entry {
...
mod tests {
    ...
    async fn assert_user_agent_modified(
        ...
        let modified_request = ua(request.unwrap_or_default()).await;
