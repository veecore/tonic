...
pub(crate) fn default() -> impl FnOnce(RawRequest<crate::body::Body>) -> Ready<RawRequest<crate::body::Body>> + Clone
{
...
{
    #[inline(always)]
    fn async_call(&mut self, input: impl Future<Output = RawRequest<Body>> + Send + 'static) -> Self::Future {
        ...
        self.inner.async_call(async move {
            modifier_fn(input.await).await
        })
    }
}